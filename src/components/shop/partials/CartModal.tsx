...
const CartModal: React.FC = () => {
  ...
  const products: Product[] = state.cartProduct;

  const cartModalOpen = () =>
    dispatch({ type: 'cartModalToggle', payload: !state.cartModal });
              ...
              <Fragment>
                {isAuthenticate() ? (
                  <Link
                    ...
                    onClick={() => cartModalOpen()}
                  >
                    Checkout ${state.cartTotalCost}.00
                  </Link>
                ) : (
                  <Link
                    className="px-4 py-2 bg-black text-white text-center cursor-pointer"
                    ...
                    onClick={() => {
                      ...
                      dispatch({
                        ...
                        payload: !state.loginSignupError,
                      });
                      dispatch({
                        type: 'loginSignupModalToggle',
                        payload: !state.loginSignupModal,
                      });
                    }}
                  >
                    Checkout ${state.cartTotalCost}.00
