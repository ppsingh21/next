...
import { OrderContext, Order } from './OrderContext';
...
const OrderRow: React.FC<{
  order: Order;
  editOrder: (oId: string, type: boolean, status: string) => void;
}> = ({ order, editOrder }) => {
  ...
  return (
    <Fragment>
      <tr className="border-b">
        ...
        <td className="hover:bg-gray-200 p-2 text-center">{order.booking}</td>
        {/* <td className="hover:bg-gray-200 p-2 text-center">
          {order.user.email}
        </td> */}
        ...
        <td className="p-2 flex items-center justify-center">
          <span
            onClick={() => editOrder(order.id, true, order.status)}
          ...
          <span
            onClick={() => deleteOrderReq(order.id, dispatch)}
            className="cursor-pointer hover:bg-gray-200 rounded-lg p-2 mx-1"
          >
            <svg
